...
import { addWrongQuestion, type Subject } from '@/lib/database'
...
export default function NewWrongQuestionPage() {
  ...
  const [, setHasDiagram] = useState(false)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (diagramBase64) {
        const base64Data = diagramBase64.replace(/^data:image\/\w+;base64,/, '')
        ...
        const file = new File([blob], 'diagram.jpg', { type: 'image/jpeg' })
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setHasDiagram(data.hasDiagram || false)
      setDiagramBase64(data.imageBase64 || null)
    } catch (error) {
      console.error('OCR error:', error)
      setOcrResult('识别失败，请尝试手动输入或重新拍照')
    } finally {
